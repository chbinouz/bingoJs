        ...
        for link in google_list:
            ...
            try:
                exp_section = soup.find('section',
                                        {'class', 'pv-profile-section experience-section ember-view'})
                exp_section.find('ul', {'class',
                ...
                for li_tagses in li_tags:
                    ...
                    try:
                        ...
                        exp = "None"
                    try:
                        location = a_tags.find('h4', {'class',
                                                      'pv-entity__location t-14 t-black--light t-normal block'}).find_all(
            ...
            try:
                semilar_profiles = soup.find('section',
                                             {"class",
                                              "artdeco-card pv-browsemap-section__wrapper ember-view"}).find(
                    "div",
                    {
                        "class",
                ...
                mongo.db.links.insert(
                    {'link': link, 'name': name, 'location': loc, 'profile_title': profile_title,
                     'experience': [],
                     'education': [], 'semilar_profiles': []})
                for ex in experiences:
                    mongo.db.links.update({'link': link}, {'$push': {
                        'experience': {'job_title': ex[0], 'company_name': ex[1], 'joining_date': ex[2],
                                       'exp': ex[3], 'location': ex[4]}}})
                for etude in education:
                    mongo.db.links.update({'link': link}, {'$push': {
                        'education': {'college_name': etude[0], 'degree_name': etude[1], 'stream': etude[2],
                                      'degree_year': etude[3]}}})
                try:
                    for prof in profiles:
                        mongo.db.links.update({'link': link}, {'$push': {'semilar_profiles': prof}})
                except:
                    pass
            csv_file.close()
        return profiles
