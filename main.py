        ...
        for linkedin_url in temporelList:
            ...
            try:
                ...
                try:
                    ...
                    CollegeName.append(college_name)
                try:
                    degree_name = edu_section.find('p', {
                        'class': 'pv-entity__secondary-title pv-entity__degree-name t-14 t-black t-normal'}).find_all(
                    ...
                    DegreeName.append(degree_name)
                try:
                    stream = edu_section.find('p', {
                        'class': 'pv-entity__secondary-title pv-entity__fos t-14 t-black t-normal'}).find_all('span')[
                        1].get_text().strip()
...
api.add_resource(ScrapeData, '/scrape')
if __name__ == "__main__":
    app.run(debug=True)

# import pandas as pd
#         linkedin = pd.DataFrame([namePerson, LocationOfPerson, ProfileTitle, NombreOfrelation, JobTitle, CompanyName, JoiningDate, Experience, CollegeName, DegreeName, Specialite,DegreeYears])
#         #CompanyName.append(" ")
