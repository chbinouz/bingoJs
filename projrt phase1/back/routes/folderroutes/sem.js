...
var sem = require('../../models/foldermodels/sem.model')

/* GET home page. */
router.get('/', function(req, res, next) {
    sem.find(function(err, data){
    if (err) throw err;
    res.json(data);
...
router.delete("/Deletehr/:id", async(req,res,next)=>{
  try {
    const profile = await sem.findByIdAndDelete(req.params.id);
